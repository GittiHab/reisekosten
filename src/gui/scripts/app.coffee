...
app.controller 'mainController', ($scope) ->
  ...
  @export = (year) ->
    save_path = remote.dialog.showSaveDialog defaultPath: year + '.xlsx', buttonLabel: 'Exportieren'
    if save_path?
      $scope.manager.exportXLS year, save_path
    return
  return
...
app.controller 'reisenController', ($scope) ->
  ...
  @delete = ->
    if confirm 'Unwiderruflich löschen?'
      ...
      deleteTravel.map (travel) -> $scope.manager.remove travel
      @deleteFields = []
    return
  return
